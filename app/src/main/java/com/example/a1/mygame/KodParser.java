...
class KodParser {
    ...
    int kodParser(String text) {
        ...
        this.text = text;
        if (text.replaceAll("\n", "").replaceAll(";", "").isEmpty()) {
            ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.wh_com);
            pause = true;
            kodERROR = true;
        } else try {
            String[] line;
            try {
                ...
                ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.ch_del);
                kodERROR = true;
                return action;
            }
            for (int i = 0; i < line.length; i++) {
                ...
                if (!cOmAnD.contains("if") && !cOmAnD.contains("while") && !cOmAnD.contains("eat") || cOmAnD.contains("repeat"))
                    ...
                    try {
                        ...
                        ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.bad_cond);
            ...
            ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.ch_com);
            kodERROR = true;
        }
        symbolslENGTH = 0;
        return action;
    ...
    private int executor(String cOmAnD, int num, int Element, String MainLine[], String text) {
        ...
        for (int g = 0; g < num; g++) {
            switch (cOmAnD) {
                case "eat":
                    if (IS_FOOD(0, 0))
                        ...
                        pause = true;
                        ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.no_food);
                    }
                    break;
                case "up":
                    if (y != 0 && !IS_LAVA(-1, 0, false)) y--;
                        ...
                        if (IS_LAVA(-1, 0, false))
                            ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.up) + ActivitySinglePlayer.AlertDialogMessage;
                    }
                    break;
                case "down":
                    if (y != square.length - 1 && !IS_LAVA(1, 0, false)) y++;
                        ...
                        pause = true;
                        if (IS_LAVA(1, 0, false))
                            ActivitySinglePlayer.AlertDialogMessage = "" + activity.getString(R.string.down) + ActivitySinglePlayer.AlertDialogMessage;
                    }
                    break;
                case "right":
                    if (x != square[0].length - 1 && !IS_LAVA(0, 1, false)) x++;
                        ...
                        if (IS_LAVA(0, 1, false))
                            ActivitySinglePlayer.AlertDialogMessage = "" + activity.getString(R.string.right) + ActivitySinglePlayer.AlertDialogMessage;
                    }
                    break;
                case "left":
                    if (x != 0 && !IS_LAVA(0, -1, false)) x--;
                        ...
                        pause = true;
                        if (IS_LAVA(0, -1, false))
                            ActivitySinglePlayer.AlertDialogMessage = "" + activity.getString(R.string.left) + ActivitySinglePlayer.AlertDialogMessage;
            ...
            if (action == ARx.length - 1) {
                ActivitySinglePlayer.AlertDialogMessage = (activity.getString(R.string.com_lim));
                kodERROR = true;
            }
            ARx[action] = x;
            ARy[action] = y;
    ...
    private boolean CONDITION_CHECKING(String text) {
        ...
        try {
            ...
            ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.bad_cond) + text;
            kodERROR = true;
        }
        return result;
    }
    ...
    private boolean CHECK_DELIMITOR(String line, String OriginLine, String text) {
        if (line.equals("")) {
            ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.exc);
            ...
            ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.after) + line.substring(0, line.indexOf(")") + 1) + activity.getString(R.string.wait);
            SELECTOR(OriginLine, text, symbolslENGTH);
    ...
    private boolean IS_COMAND_EXIST(String cOmAnD, String OriginLine, String text) {
        if (!cOmAnD.equals("up") && !cOmAnD.equals("down")
            ...
            ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.unknown) + cOmAnD;
            SELECTOR(OriginLine, text, symbolslENGTH);
            kodERROR = true;
            return true;
        } else return false;
    ...
    private int IS_NUM_EXIST(String OriginLine, String line) {
        ...
        try {
            ...
            ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.unkn_num);
    ...
    private boolean IS_LAVA(int dY, int dX, boolean justCheck) {
        ...
        if (dY < 0 || dY > square.length - 1) {
            is_LAVA = true;
            if (!justCheck)
                ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.no_sq);
        } else if (dX < 0 || dX > square[dY].length - 1) {
            is_LAVA = true;
            if (!justCheck)
                ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.no_sq);
            ...
            is_LAVA = true;
            if (!justCheck)
                ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.lava);
        } else if (square[dY][dX].ID_NUMBER == 2) {
            is_LAVA = true;
            if (!justCheck)
                ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.kisl);
        } else if (!square[dY][dX].isVISIBLE) {
            is_LAVA = true;
            if (!justCheck)
                ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.sq_dis);
        }
        return (is_LAVA);
    }
    ...
    private void LOOP(int LoopElement, String MainLine[], String text) {
        ...
        if (!test.equals("){")) {
            ActivitySinglePlayer.AlertDialogMessage = (activity.getString(R.string.misd_op));
    ...
    private int idetifyBODY(String MainLine[], int LoopElement, String text) {
        ...
        ActivitySinglePlayer.AlertDialogMessage = (activity.getString(R.string.misd_cl));
        SELECTOR(MainLine[LoopElement], text, symbolslENGTH);
        kodERROR = true;
    ...
    private int IEidetifyBODY(String MainLine[], int LoopElement, String text) {
        ...
        while (!MainLine[elsenum.get(elnum)].replaceAll("\n", "").equals("}")) {
            if (IS_ELSE_OKAY(MainLine[elsenum.get(elnum)])) {
                ...
                ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.wait_else) + MainLine[elsenum.get(elnum)];
    ...
    private boolean IS_ELSE_OKAY(String line) {
        ...
        if (line.contains("if")) {
            if (line.substring(line.indexOf("}") + 1, line.indexOf("i")).trim().equals("else"))
                ...
                ActivitySinglePlayer.AlertDialogMessage = activity.getString(R.string.bad_cond) + line;
