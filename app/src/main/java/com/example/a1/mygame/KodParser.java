package com.example.a1.mygame;
 ...
 class KodParser {
    ...
    int kodParser(String text) {
        ...
        if (text.replaceAll("\n","").replaceAll(";","").isEmpty()){
            ...
            for (int i = 0; i < line.length; i++) {
                ...
                symbolslENGTH+=MainLine[i].length()+1;
            }
            symbolslENGTH=0;
        }
        catch(Throwable t){
        ...
        for (int g=0;g<num;g++) {
            switch (cOmAnD) {
                case "up":
                    ...
                    else if (!pause){pause =true;  MyGame.AlertDialogMessage  = "Вверху клеток нет.";}break;
                case "down":
                    if(y!=square.length-1&&!IS_LAVA(1,0))y++;
                    else if (!pause){ pause =true; MyGame.AlertDialogMessage  = "Внизу клеток нет.";}break;
                case "right":
                    if(x!=square[0].length-1&&!IS_LAVA(0,1))x++;
                    else if (!pause){ pause =true; MyGame.AlertDialogMessage  ="Справа клеток нет.";}break;
                case "left":
                    if(x!=0&&!IS_LAVA(0,-1))x--;
                    else if (!pause){ pause =true; MyGame.AlertDialogMessage  ="Слева клеток нет.";}break;
                case "repeat":
    ...
    private void LOOP(int LoopElement,String MainLine[]){
        ...
        if(!test.equals("){")) {
            MyGame.AlertDialogMessage  = ("Пропущен открывающий ТЭГ!");
            kodERROR=true;
        }
    ...
    private int idetifyBODY(String MainLine[],int LoopElement){
        ...
        MyGame.AlertDialogMessage =("Провущен закрывающий ТЭГ!");
        kodERROR=true;
        return -1;
    }
    ...
    private boolean IS_LAVA(int y, int x){
        switch (x) {
            ...
            case (1): x = this.x + 1; break;
            default: x = this.x;break;
        }
        switch (y) {
            case (-1):y = this.y - 1; break;
            case (1): y = this.y + 1; break;
            default: y = this.y;break;
        }
        if (square[y][x].ID_NUMBER==1){
            pause = true;MyGame.AlertDialogMessage = "Впереди ЛАВА!";
        }
        return (pause);
    }
    boolean isKodERROR() {
